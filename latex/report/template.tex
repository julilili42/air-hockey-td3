...
\team{alphabet-td3: Julian Jurcevic}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
    ...
    \item A three-stage curriculum strategy that combines scripted opponents with self-play to mitigate non-stationarity and improve generalization.
...
\end{itemize}
The primary objective is to develop a competitive agent for the final tournament. The full source code and training setup are publicly available~\cite{jurcevic2026airhockey}.
...
We consider both uniform and prioritized sampling~\cite{schaul2016per}, where transitions are sampled proportional to their TD error
\[
...
We evaluate Gaussian, Ornstein--Uhlenbeck~\cite{lillicrap2015ddpg}, and pink noise~\cite{eberhard2023pink}.
...
Curriculum training~\cite{bengio2009curriculum} is used to obtain a versatile competition agent 
...
Figure~\ref{fig:stage1} shows that weak-only training quickly improves performance but does not transfer to the strong opponent.  
...
Stage III yields the strongest overall competition-ready policy (Figure~\ref{fig:stage3}).
...
A plausible explanation is that the temporal correlation of OU noise produces smoother action trajectories. In a fast-paced physics environment, uncorrelated perturbations can disrupt otherwise stable motor sequences.
...
\begin{figure}[!t]
    ...
    \begin{subfigure}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/final_12k_combined.pdf}
        \caption{Stage III: Final curriculum refinement.}
        \label{fig:stage3}
    \end{subfigure}

    \caption{
    Training progression across curriculum stages.
    Blue: return (100-episode MA). Red/green: win rate.
    }
...
Curriculum learning was the most impactful design choice. Without it, the agent overfits to weak-opponent strategies (Figure~\ref{fig:stage1}). Staged scheduling resolves this while retaining base competence.

OU noise yielded the clearest gain ($+8\%$ against strong, Table~\ref{tab:noise_ablation}), likely due to temporally correlated exploration producing smoother actions suited to fast physics dynamics. PER consistently hurt performance (Table~\ref{tab:final_eval}), possibly because non-stationary opponents amplify the variance of priority-based sampling. Self-play reduced benchmark scores but is retained for tournament robustness; pool diversity remains a limitation as snapshots converge over training.

Overall, TD3 with curriculum learning, OU noise, and self-play produces a stable, competitive agent for the air hockey environment.
